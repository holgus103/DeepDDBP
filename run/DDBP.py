...
PARTITION = 0.66
SET_SIZE = 600
EXPERIMENT = "no_trump_l_104_52_13_p104_c_2_inbalanced_draws_test"
...
finetuning_outputs = numpy.load(path + "finetuningtest_outputs.npy")
comparables = dp.labeled_dictionary(data, labels, 3);
(samples_l, samples_r, outputs, diffs) = dp.generate_balanced_classes(data, labels, 200, 400, 200);
...
# train whole network
c.train(data_batches_l, data_batches_r, outputs_batches, 0.0001, 15000, 0.0001, path +"/finetuning", samples_l, samples_r, outputs, diffs, test_samples_l, test_samples_r, test_outsputs, test_diffs, test_data, test_labels, finetuning_outputs, comparables, dp.suit_count_for_params(TRAIN_NO_TRUMP, TRAIN_TRUMP), dp.suit_count_for_params(TEST_NO_TRUMP, TEST_TRUMP), models.Model.mse_loss, 25, experiment_name);
